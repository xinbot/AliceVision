...
int aliceVision_main(int argc, char * argv[])
{
    ...
    requiredParams.add_options()
        ...
        ("outputFolder,o", po::value<fs::path>(&outputFolderPath)->required(),
         "Output Folder.")
        ;
    ...
    if(!inputExpression.empty() && std::find(sfmSupportedExtensions.begin(), sfmSupportedExtensions.end(), inputExt) != sfmSupportedExtensions.end())
    {
        ...
        for(auto& viewIt : ViewPaths)
        {
            ...
            const std::string outputfilePath = (outputFolderPath / (std::to_string(viewId) + outputExt)).generic_string();
        ...
        const std::string sfmfilePath = (outputFolderPath / "sfmData.abc").generic_string();
    ...
    else
    {
        ...
        for(const std::string& inputFilePath : filesStrPaths)
        {
            ...
            const std::string outputExt = extension.empty() ? fileExt : (std::string(".") + extension);
            const std::string outputFilePath = (outputFolderPath / (filename + outputExt)).generic_string();
